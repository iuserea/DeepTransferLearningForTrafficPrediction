        ...
        if opt.transfer == 1:
            ...
            if opt.traffic == 'sms':
                if opt.target_type == 'call':
                    model.load_state_dict(torch.load('/data/call/base_200.pt'))
                elif opt.target_type == 'internet':
                    model.load_state_dict(torch.load('/data/internet/base_200.pt'))
                else:
                    raise NotImplementedError()
            if opt.traffic == 'call':
                if opt.target_type == 'sms':
                    model.load_state_dict(torch.load('/data/sms/base_200.pt'))
                elif opt.target_type == 'internet':
                    model.load_state_dict(torch.load('/data/internet/base_200.pt'))
                else:
                    raise NotImplementedError()
